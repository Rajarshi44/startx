...
export default function TavusInterviewModal({
  ...
  const endInterview = async () => {
    ...
    try {
      ...
      await fetch(`/api/tavus/conversation?conversationId=${conversation.conversation_id}&civicId=${civicId}`, {
        method: "DELETE",
      })
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl h-[80vh] p-0 bg-[#111111] border-[#ffcb74]/20">
        ...
        <div className="flex-1 p-6 pt-0">
          {step === "setup" && (
            <div className="space-y-6">
                ...
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold mb-2 text-[#f6f6f6]">{jobPosting?.title || "Job Details"}</h3>
                      ...
                      {jobPosting?.skills_required && (
                        <div className="flex flex-wrap gap-2">
                          {jobPosting?.skills_required.slice(0, 5).map((skill: string, index: number) => (
                            <Badge key={index} className="bg-[#ffcb74]/20 text-[#ffcb74] text-xs">
                              {skill}
                            </Badge>
                          ))}
                        </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div className="p-3 bg-[#111111] rounded-lg border border-[#ffcb74]/20">
                          ...
                          <div className="text-sm text-[#f6f6f6]/70">Powered</div>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <Card className="bg-[#2f2f2f] border-[#ffcb74]/20">
                <CardContent className="p-6">
                  <h3 className="text-lg font-semibold mb-4 text-[#f6f6f6]">Before You Start</h3>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      <CheckCircle className="h-5 w-5 text-[#ffcb74]" />
                      <span className="text-[#f6f6f6]">Ensure you have a stable internet connection</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <CheckCircle className="h-5 w-5 text-[#ffcb74]" />
                      <span className="text-[#f6f6f6]">Test your camera and microphone</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <CheckCircle className="h-5 w-5 text-[#ffcb74]" />
              ...
              <div className="flex gap-3">
                <Button onClick={handleClose} variant="outline" className="flex-1 bg-[#111111] border-[#ffcb74] text-[#f6f6f6] hover:bg-[#111111]/80">
                  Cancel
                </Button>
                <Button onClick={startInterview} disabled={isLoading} className="flex-1 bg-[#ffcb74] hover:bg-[#ffcb74]/80 text-[#111111] font-semibold">
                  {isLoading ? (
                    <>
          ...
          {step === "creating" && (
            <div className="flex flex-col items-center justify-center h-full space-y-6">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold mb-2 text-[#f6f6f6]">Setting up your interview...</h3>
                <p className="text-[#f6f6f6]/70">Please wait while we prepare your AI interviewer</p>
              </div>
            </div>
          )}

          {step === "interview" && conversation && (
            <div className="h-full flex flex-col">
              {/* Interview Controls */}
              <div className="flex items-center justify-between p-4 bg-[#2f2f2f] rounded-lg mb-4 border border-[#ffcb74]/20">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
                    <span className="text-sm font-medium text-[#f6f6f6]">LIVE</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock className="h-4 w-4 text-[#f6f6f6]/70" />
                    <span className="text-sm font-mono text-[#f6f6f6]">{formatDuration(duration)}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    className={isVideoEnabled ? "bg-[#ffcb74] text-[#111111]" : "bg-[#111111] text-[#f6f6f6] border-[#ffcb74]"}
                  >
                  ...
                  <Button
                    ...
                    className={isAudioEnabled ? "bg-[#ffcb74] text-[#111111]" : "bg-[#111111] text-[#f6f6f6] border-[#ffcb74]"}
                  >
                    {isAudioEnabled ? <Mic className="h-4 w-4" /> : <MicOff className="h-4 w-4" />}
                  </Button>
                  <Button size="sm" variant="destructive" onClick={endInterview} className="bg-red-600 hover:bg-red-700">
              ...
              <div className="flex-1 bg-black rounded-lg overflow-hidden border border-[#ffcb74]/20">
                <iframe
                  ref={iframeRef}
          ...
          {step === "completed" && (
            <div className="flex flex-col items-center justify-center h-full space-y-6">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold mb-2 text-[#f6f6f6]">Interview Completed!</h3>
                <p className="text-[#f6f6f6]/70 mb-4">
                  Thank you for completing the interview. Your responses have been recorded and will be reviewed.
                </p>
                <div className="text-sm text-[#f6f6f6]/50">Duration: {formatDuration(duration)}</div>
              </div>
              <Button onClick={handleClose} className="bg-[#ffcb74] hover:bg-[#ffcb74]/80 text-[#111111] font-semibold">
                Close
              </Button>
            </div>
          )}
